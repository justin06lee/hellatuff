...
function BrushIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" style={{ height: 20, width: 20, fill: "none", stroke: "currentColor" }}>
      <path
...
function ResizeHandleIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" style={{ height: 14, width: 14, fill: "none", stroke: "currentColor" }}>
...
function UndoIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      style={{ height: 18, width: 18, fill: "none", stroke: "currentColor" }}
    >
...
export function DrawingWindow(props: DrawingWindowProps) {
  ...
  const theme = useAutoTheme(undefined);
  ...
  async function exportBothVariants() {
    const canvas = sourceCanvasRef.current;
    if (!canvas) {
      throw new Error("No canvas available");
    }

    const lightDataUrl = canvas.toDataURL("image/png");
    const darkDataUrl = createDarkVariantDataUrl(canvas);
    const name = `drawing-${Date.now()}`;
    return { lightDataUrl, darkDataUrl, name };
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      console.error("drawing save failed", saveError);
      ...
      >
        ...
        <div
          style={{
            ...
            borderBottom: "1px solid var(--hlt-border)",
            padding: "0.75rem 1rem",
            background: active ? "var(--hlt-surface-alt)" : "var(--hlt-surface)",
          }}
          onPointerDown={(event) => {
        ...
        >
          <div>
            <p style={{
              ...
              Drawing Window{windowId != null ? ` #${windowId}` : ""}
            </p>
          </div>
          <button
            type="button"
        ...
        <div style={{
          ...
          <div
            style={{
              minHeight: 0,
              borderRight: "1px solid var(--hlt-border)",
              padding: "0.75rem",
          ...
          >
            ...
            <div style={{
              ...
              <span style={{ minWidth: 48, textAlign: "center", color: "var(--hlt-foreground)" }}>
                {Math.round(zoom * 100)}%
              </span>
              <button
                type="button"
              ...
              >
                <canvas
                  ...
                  style={{
                    ...
                    cursor: spacePressed || isPanning ? "grab" : "crosshair",
          ...
          <div style={{
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    border: `1px solid ${tool === "brush" ? "var(--hlt-foreground)" : "var(--hlt-border)"}`,
                    background: tool === "brush" ? "var(--hlt-foreground)" : "var(--hlt-background)",
                    color: tool === "brush" ? "var(--hlt-background)" : "var(--hlt-foreground)",
                    cursor: "pointer",
                ...
                <button
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    border: `1px solid ${tool === "eraser" ? "var(--hlt-foreground)" : "var(--hlt-border)"}`,
                    background: tool === "eraser" ? "var(--hlt-foreground)" : "var(--hlt-background)",
                    color: tool === "eraser" ? "var(--hlt-background)" : "var(--hlt-foreground)",
                    cursor: "pointer",
                    transition: "background-color 0.15s ease",
                  }}
                >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        width: 36,
                        borderRadius: "50%",
                        border: `1px solid ${selected ? "var(--hlt-foreground)" : "var(--hlt-border)"}`,
                        backgroundColor: presetColor.display,
                        cursor: "pointer",
                        transition: "transform 0.15s ease",
                        boxShadow: selected ? "0 0 0 2px color-mix(in srgb, var(--hlt-foreground) 20%, transparent)" : "none",
                      }}
                    >
                      <span style={{
                        position: "absolute",
                ...
                <label
                  style={{
                    ...
                    background: theme === "dark" ? "#000000" : "#ffffff",
                    color: "var(--hlt-foreground)",
                    boxShadow: !PRESET_COLORS.some((preset) => preset.source === color.toLowerCase())
                      ? "0 0 0 2px color-mix(in srgb, var(--hlt-foreground) 20%, transparent)"
                      : "none",
                  }}
                  title="Custom color"
                >
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    style={{ height: 16, width: 16, fill: "none", stroke: "currentColor", color: "var(--hlt-foreground)" }}
                  >
                    <path
                      d="M12 5v14M5 12h14"
                      strokeLinecap="round"
                ...
                {BRUSH_SIZES.map((brushSize) => (
                  <button
                    ...
                    style={{
                      ...
                      alignItems: "center",
                      justifyContent: "center",
                      border: `1px solid ${size === brushSize ? "var(--hlt-foreground)" : "var(--hlt-border)"}`,
                      padding: "0 0.75rem",
                      background: size === brushSize ? "var(--hlt-foreground)" : "var(--hlt-background)",
                      color: size === brushSize ? "var(--hlt-background)" : "var(--hlt-foreground)",
                      cursor: "pointer",
                      transition: "background-color 0.15s ease",
                    }}
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button
                  ...
                  style={{
                    ...
                    background: "var(--hlt-background)",
                    color: "var(--hlt-foreground)",
                    cursor: historyState.undo === 0 ? "not-allowed" : "pointer",
                    opacity: historyState.undo === 0 ? 0.4 : 1,
                    transition: "background-color 0.15s ease",
                  }}
                >
                  <UndoIcon />
                ...
                <button
                  ...
                  style={{
                    ...
                    background: "var(--hlt-background)",
                    color: "var(--hlt-foreground)",
                    cursor: historyState.redo === 0 ? "not-allowed" : "pointer",
                    opacity: historyState.redo === 0 ? 0.4 : 1,
                    transition: "background-color 0.15s ease",
                  }}
                >
                  <RedoIcon />
            ...
            {/* Error */}
            {error ? (
              <p style={{ fontSize: "0.875rem", color: "var(--hlt-danger)" }}>{error}</p>
            ) : null}
            ...
            <button
              ...
              style={{
                width: "100%",
                border: "1px solid var(--hlt-foreground)",
                background: "var(--hlt-foreground)",
                padding: "0.5rem 1rem",
                fontSize: "0.875rem",
                fontWeight: 500,
                color: "var(--hlt-background)",
                cursor: isSaving || saveLocked ? "not-allowed" : "pointer",
                opacity: isSaving || saveLocked ? 0.6 : 1,
              }}
            >
              {isSaving
                ? "Saving image..."
        ...
        <div
          style={{
            ...
            alignItems: "center",
            justifyContent: "center",
            borderLeft: "1px solid var(--hlt-border)",
            borderTop: "1px solid var(--hlt-border)",
